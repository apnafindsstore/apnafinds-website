(() => {
  ...
  const PHOTO_MIGRATION_KEY = "apnafinds_real_photo_catalog_v4";
  ...
  function migrateLegacyProductPhotos() {
    if (localStorage.getItem(PHOTO_MIGRATION_KEY) === "done") {
      return;
    }

    const migrateArray = (key) => {
      ...
      const migrated = value.map(item => {
        ...
        const replacement = PRODUCT_PHOTOS[slugify(name)];
        return replacement
          ? { ...item, image: replacement, imageUrl: replacement }
          : item;
      });
    ...
    migrateArray(KEYS.products);
    migrateArray(KEYS.cart);
    migrateArray(KEYS.cartLegacy);

    localStorage.setItem(PHOTO_MIGRATION_KEY, "done");
  }

  function loadProducts() {
    migrateLegacyProductPhotos();
